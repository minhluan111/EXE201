...
function Compress-FileItem {
    ...
    try {
        $sourceImg = [System.Drawing.Image]::FromFile($fileItem.FullName)
...
foreach ($item in $items) {
    ...
    elseif ($name -like "*dừa quế hoa*" -or $name -like "*d?a qu? hoa*") {
        Compress-FileItem $item "$menuDir\nuoc_dua_que_hoa.jpg" 800 800 85
    }
    elseif ($name -like "*lựu đỏ*" -or $name -like "*l?u d?*") {
        Compress-FileItem $item "$menuDir\tra_luu_do.jpg" 800 800 85
    }
    elseif ($name -like "*ổi hồng*" -or $name -like "*?i h?ng*") {
        Compress-FileItem $item "$menuDir\tra_oi_hong.jpg" 800 800 85
    }
    elseif ($name -eq "nước_" -or $name -like "n?c_*" -or $name -like "nước*") {
        Compress-FileItem $item "$menuDir\tra_trai_cay.jpg" 800 800 85
    }
    # Tables
    elseif ($name -like "*bàn 2 người*" -or $name -like "*b?n 2 ngu?i*") {
        if ($name -like "*(1)*") {
    ...
    elseif ($name -like "*bàn 4 người*" -or $name -like "*b?n 4 ngu?i*") {
        if ($name -like "*(1)*") {
    ...
    elseif ($name -like "*bàn 8 người*" -or $name -like "*b?n 8 ngu?i*") {
        Compress-FileItem $item "$tablesDir\ban_8_nguoi.jpg" 800 600 85
    }
    # Decor / Space
    elseif ($name -like "*không gian quán*" -or $name -like "*khng gian qun*") {
        if ($name -like "*(1)*") {
    ...
    elseif ($name -like "*không gian*" -or $name -like "*khng gian*") {
